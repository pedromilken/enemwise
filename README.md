...
- **Tutor opcional com o LLM que o estudante quiser:** Anthropic (Claude), OpenAI (GPT), Google (Gemini) ou qualquer serviço compatível com a API da OpenAI (DeepSeek, Qwen, modelos locais), informando o endereço base. A chave fica só na aba do navegador e o uso é cobrado na conta de quem a informou.
- **Resolução comentada** após cada resposta e nas revisões, com link para a resolução dos professores do Curso Objetivo na página da prova (site externo; o texto deles não é copiado, por direitos autorais). Gerada uma vez no pipeline com `enemwise resolucoes` (API da Anthropic, cache retomável em `out/resolucoes.json`) e levada ao app pelo `merge`.
- **Dificuldade percebida e revisão espaçada:** o estudante marca a questão como fácil, média ou difícil; erros voltam em 1 dia, difíceis em 3, médias em 7, fáceis em 21. Cerca de um terço do treino é revisão quando há questões vencidas, e o app avisa quantas há.
- **Evolução no tempo:** a estimativa da plataforma dia a dia (a partir do θ registrado antes de cada resposta), e o estudante registra notas de Enem anteriores e simulados por área; o app compara com a estimativa atual e a meta, com variação e gráfico. Entra no relatório e sincroniza entre aparelhos.
